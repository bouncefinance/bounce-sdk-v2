...
const PlaceBidButton = ({
	...
	loading,
	poolInfo,
}: PlaceBidButtonProps): JSX.Element => {
	...
	const currencyBidAmount = CurrencyAmount.fromAmount(
		poolInfo.currencyAmountTotal1.currency,
		bidAmount,
	)
	const [approvalState, approveCallback] = useApproveCallback(
		currencyBidAmount,
		fixedSwapERC20Contract?.address,
		true,
	)
	...
	return (
		<LoadingButton
			...
			loading={loading}
			disabled={
				!bidAmount || !poolInfo.whitelistData?.isUserInWhitelist || poolInfo.whitelistData?.loading
			}
			onClick={onClick}
		>
			{loading ? (
