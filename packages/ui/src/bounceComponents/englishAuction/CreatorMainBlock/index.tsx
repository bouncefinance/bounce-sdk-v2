...
const CreatorMainBlock = (): JSX.Element => {
	...
	const isAllTokenSwapped = useMemo(
		() => !!poolInfo?.currentBidder && !isZero(poolInfo.currentBidder),
		[poolInfo],
	)

	const platformFeeText = useMemo(() => {
		if (!poolInfo || !poolInfo.currentBidderAmount1?.greaterThan('0')) return '-'
		return new BigNumber(poolInfo.currentBidderAmount1.toExact())
			.multipliedBy(TX_FEE_RATIO)
			.toFixed(2)
	}, [poolInfo])
				...
				<Stack spacing={10}>
					<PoolInfoItem title="Current Highest Bid" tip="The current highest bid for the auction">
						<Stack direction="row" spacing={6} alignItems="center">
							...
							<TokenImage
								src={
									poolInfo.token1.largeUrl || poolInfo.token1.smallUrl || poolInfo.token1.thumbUrl
								}
					...
					<PoolInfoItem
						...
						tip="The wallet address that fund raised will send to."
					>
