		...
		poolSize: Yup.number()
			...
			.test(
				...
				(value) =>
					!value ||
					(balance
						? !balance.lessThan(CurrencyAmount.fromAmount(balance.currency, value) || ZERO)
						: false),
			),
			...
			.when('allocationStatus', {
				...
				then: Yup.number()
					...
					.test('DIGITS_LESS_THAN_6', 'Should be no more than 6 digits after point', (value) => {
						const _value = new BigNumber(value || 0).toFixed()
						return (
							!_value || !String(_value).includes('.') || String(_value).split('.')[1]?.length <= 6
						)
					})
							...
							<Box>
								...
								>
									...
									{values.tokenFromSymbol && (
										<Typography>Balance: {balance?.toSignificant() || '-'}</Typography>
									)}
								</Stack>
											...
											<>
												<Button
													...
													onClick={() => {
														setFieldValue(
															'poolSize',
															balance?.toSignificant(64, { groupSeparator: '' }),
														)
													}}
												>
													Max
