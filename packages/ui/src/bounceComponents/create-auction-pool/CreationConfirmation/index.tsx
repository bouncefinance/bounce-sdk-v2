...
const CreatePoolButton = (props?: { onSuccess?: () => void }) => {
	...
	const { auctionChainId, auctionInChainId, account } = values
	...
	const auctionPoolSizeAmount = useMemo(
		() =>
			currencyFrom && values.poolSize
				? CurrencyAmount.fromAmount(currencyFrom, values.poolSize)
				: undefined,
		[currencyFrom, values.poolSize],
	)
				...
				transactionReceipt.then((curReceipt) => {
					...
					if (poolId) {
						...
						show(DialogTips, {
							...
							title: 'Oops..',
							content:
								'The creation may have failed. Please check some parameters, such as the start time',
						})
						reject()
					}
				})
	...
	}, [chainConfigInBackend?.id, createFixedSwapPool])
	...
	const confirmBtn: {
		...
		if (
			...
			!auctionPoolSizeAmount ||
			auctionPoolSizeAmount.greaterThan(auctionAccountBalance)
		) {
			return {
...
const CreationConfirmation = () => {
	...
	const { auctionType, auctionInChainId } = values
					...
					<Stack spacing={24}>
						<ConfirmationInfoItem title="Chain">
							<Box sx={{ display: 'flex', alignItems: 'center' }}>
								...
								/>
								<Typography sx={{ ml: 4 }}>
									{auctionInChainId ? ChainListMap[auctionInChainId]?.name : ''}
								</Typography>
							</Box>
							...
							<Stack spacing={15}>
								...
								<ConfirmationInfoItem title="Token symbol">
									<Stack direction="row" spacing={8} alignItems="center">
										<TokenImage
											alt={values.tokenFrom.symbol}
											src={values.tokenFrom.logoURI}
											size={20}
										/>
						...
						<Box>
							...
							<Stack spacing={15}>
								...
								<ConfirmationInfoItem title="To">
									<Stack direction="row" spacing={8} alignItems="center">
										<TokenImage
											alt={values.tokenTo.symbol}
											src={values.tokenTo.logoURI}
											size={20}
										/>
								...
								<ConfirmationInfoItem title="Allocation per Wallet">
									<Typography>
										{values.allocationStatus === AllocationStatus.NoLimits
											? 'No'
											: `Limit ${Number(values.allocationPerWallet).toLocaleString()} ${
						...
						<Box>
							...
							<Stack spacing={15}>
								<ConfirmationInfoItem title="Pool duration">
									<Typography>
										From {values.startTime?.format('MM.DD.Y HH:mm')} - To{' '}
										{values.endTime?.format('MM.DD.Y HH:mm')}
									</Typography>
								</ConfirmationInfoItem>
