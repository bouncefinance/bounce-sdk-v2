...
import { CreationStep, IReleaseData, IReleaseType, ParticipantStatus } from '../types'
...
export const AddIReleaseTypeAdvanced = ({ hideRefundable }: { hideRefundable?: boolean }) => {
	...
	const validationSchema = Yup.object({
		poolName: Yup.string()
			.max(30, 'Pool name should be less than 30 characters')
		...
		startTime: Yup.date()
			...
			.test(
				'EARLIER_THAN_END_TIME',
				'Please select a time earlier than end time',
				(value, context) => {
					return (
		...
		endTime: Yup.date()
			...
			.typeError('Please select a valid time')
			.test(
				'LATER_THAN_START_TIME',
				'Please select a time later than start time',
				(value, context) => {
					return (
						...
						test: (input, context) => {
							if (moment(input) < moment()) {
								return context.createError({
									message: 'Please select a time earlier than current time',
								})
							}
							if (
								!(
								...
								return context.createError({
									message: 'Please select a time later than start time and end time',
								})
							}
							return true
						},
					...
					.test({
						...
						test: (input, context) => {
							...
							if (
								...
								return context.createError({
									message: 'Please select a time later than linear unlocking end time',
								})
							}
							return true
						},
		...
		fragmentReleaseSize: Yup.string().when('releaseType', {
			...
			then: Yup.string().test(
				'TEST_FRAGMENT_TOTAL',
				'Release ratio must add up to 100%',
				(_, context) => {
		...
		participantStatus: Yup.string().oneOf(
				...
				<Formik
					...
					onSubmit={async (formValues) => {
						...
						if (Number(formValues.releaseType) === IReleaseType.Linear) {
							releaseDataArr = [
								{
									startAt: formValues.linearUnlockingStartTime,
									endAt: formValues.linearUnlockingEndTime,
								},
							]
						...
						valuesDispatch({
							...
							payload: {
								...
								shouldDelayUnlocking:
									Number(formValues.releaseType) === IReleaseType.Cliff ? true : false,
							},
						})
					}}
				>
						...
						return (
							<Form>
								...
								<Box sx={{ mt: 38 }}>
									...
									<FormHelperText error={!!errors.fragmentReleaseSize}>
										{errors.fragmentReleaseSize}
									</FormHelperText>
								</Box>
									...
									<Box sx={{ mt: 38, mb: 34 }}>
										...
										<Box sx={{ display: 'flex', alignItems: 'center' }}>
											<Typography variant="body1">
												Auction will be refundable before the end time
											</Typography>
								...
								<Box sx={{ mt: 38, mb: 34 }}>
									...
									<FormHelperText error={!!errors.participantStatus}>
										{errors.participantStatus}
									</FormHelperText>
										...
										<Button
											...
											onClick={() => {
												...
												valuesDispatch({
													...
													payload: {
														activeStep: CreationStep.AUCTION_PARAMETERS,
													},
												})
											}}
										>
