			...
			{valuesState.activeStep !== CreationStep.CREATION_CONFIRMATION ? (
				<Box sx={{ pb: 48, maxWidth: 710, width: '100%' }}>
					{valuesState.activeStep === CreationStep.TOKEN_INFORMATION && (
						<TokenERC721InformationForm />
					)}

					{valuesState.activeStep === CreationStep.AUCTION_PARAMETERS && (
						<Auction721ParametersForm />
					)}

					{valuesState.activeStep === CreationStep.ADVANCED_SETTINGS && (
						<AdvancedSettingsForm hideRefundable />
					)}
				</Box>
			) : (
