...
type Payload = {
	...
	// }
	[ActionType.Update]: Partial<AuctionPool>
	...
	switch (action.type) {
		...
		case ActionType.CommitAdvancedSettings:
			return {
				...
				releaseType:
					action.payload.releaseType !== undefined
						? action.payload.releaseType
						: IReleaseType.Cliff,
				releaseDataArr: action.payload.releaseDataArr || [],
				delayUnlockingTime: action.payload.delayUnlockingTime,
				enableReverse:
					action.payload.enableReverse === undefined ? true : action.payload.enableReverse || false,
				activeStep: state.activeStep + 1,
				completed: { ...state.completed, [state.activeStep]: true },
...
const ValuesProvider = ({
	children,
	state,
}: PropsWithChildren<{ state?: Partial<AuctionPool> }>) => {
	const [defaultState, dispatch] = useReducer(reducer, initialValues)
	useEffect(() => {
		state && dispatch({ type: ActionType.Update, payload: state })
	}, [state])
	return useMemo(
		() => (
			<ValuesStateContext.Provider value={{ ...defaultState, ...state }}>
