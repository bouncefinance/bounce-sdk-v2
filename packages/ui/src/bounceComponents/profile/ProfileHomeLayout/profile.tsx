	...
	return (
		...
		>
			...
			>
				...
				<H6
					sx={{ fontSize: { xs: 16, sm: 14 }, color: '#2B51DA' }}
				>{`#${personalInfo?.fullNameId}`}</H6>
			</Stack>
			<Row mt={24} gap={4}>
		...
		() => [
			...
			// },
			{
				...
				components: personalInfo ? (
					<TokenAuction userInfo={personalInfo} tokenType={BackedTokenType.TOKEN} />
				) : (
					<></>
				),
			},
			{
				...
				href: 'NFT',
				components: personalInfo ? (
					<TokenAuction userInfo={personalInfo} tokenType={BackedTokenType.NFT} />
				) : (
					<></>
				),
			},
	...
	return (
		<>
			<Box sx={{ position: 'relative', width: '100%', ...styles.contain }}>
					...
					{tabsList?.map((item) => {
						return (
							<Typography
								...
								sx={{
									...styles.menu,
									...(hasActive(item?.href) ? styles.menuActive : ({} as any)),
								}}
							>
				...
				/>
				<Box sx={{ background: '#FFFFFF', borderRadius: 20, minHeight: 711 }}>
					{currentTypeRender}
				</Box>
			</Box>
		</>
