...
const EditInfo: React.FC<IEditInfoProps> = ({ userInfoEmail, userId, handleEmailChange }) => {
	const [mode, setMode] = useState<'unset' | 'set' | 'input1' | 'input2'>(
		userInfoEmail ? 'set' : 'unset',
	)
	...
	const { run: handleChangeEmail, loading } = useRequest(
		async (params: IChangeEmailParams) => changeEmail(params),
		{
			manual: true,
			onSuccess: (response) => {
				const { code } = response
	...
	const { run: sendVerifyCode } = useRequest(
		async (email: string) => verifyCode({ email, codeType: 1 }),
		{
			manual: true,
			onSuccess: (response) => {
				const { code } = response
										...
										mode === 'set' ? (
											<Box display="flex" alignItems="center">
												...
												>
													...
													<path
														d="M7.5 12.7181L10.6702 15.5L16.5 8.5"
														stroke="white"
														strokeLinecap="round"
													/>
												</svg>
											...
											<Button
												sx={{
													...
													height: 30,
													color:
														errors.email || !values.email || showCountDown
															? 'var(--ps-text-2)'
															: 'var(--ps-black)',
													fontWeight: 600,
													fontSize: 14,
