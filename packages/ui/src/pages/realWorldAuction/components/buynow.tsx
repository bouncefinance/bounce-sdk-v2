...
import {
	...
	useValuesDispatch,
	useValuesState,
} from '@/bounceComponents/real-world-collectibles/ValuesProvider'
...
const BuynowContent = () => {
	...
	const {
		...
		async ({ current, pageSize = defaultPageSize }) => {
			...
			const filterCategories = filterData.find(
				(single: FilterSearchConfig) => single.label === 'Categories',
			)
			marketList.map((item) => {
				let isPass = true
				...
				if (isPass && values.status) {
					...
					if (values.status === 'Upcoming') {
						...
						if (item.startTime && item.endTime) {
							isPass = !!(
								Number(item.startTime) * 1000 <= nowTime && Number(item.endTime) * 1000 > nowTime
							)
	...
	return (
		...
		>
			...
			>
				...
				>
					You can buy now or go to the details page to make an offer. You are buying for a physical
					backed NFTs and you can redeem it post deal done.
				</Typography>
			</Box>
			{!isSm && (
				<Marketplace
