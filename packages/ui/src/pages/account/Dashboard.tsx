...
import {
	useDashboardStat,
	useDashboardUserCollect,
	useDashboardUserCreated,
} from 'bounceHooks/account/useDashboard'
								...
								<Box display="flex" alignItems={'center'}>
									<Box sx={{ position: 'relative', width: '120px' }}>
										{!userInfo?.avatar ? (
											...
											<AccountAvatar
												src={userInfo?.avatar?.fileThumbnailUrl || userInfo?.avatar?.fileUrl}
											/>
										)}
										{userInfo?.isVerify && (
											<VerifiedIcon
						...
						{isSm && (
							<Box pt={26}>
								...
								<Stack
									mt={22}
									mb={24}
									sx={{ flexDirection: 'row', gap: 8, justifyContent: 'center' }}
								>
									{social}
								</Stack>
						...
						<Box padding={isSm ? '34px 26px' : '50px 60px'}>
							...
							>
								...
								<DashboardStatCard
									...
									value={formatGroupNumber(dashboardStat?.createdCount || 0)}
								/>
								<DashboardStatCard
...
function PendingClaimAuctionsList() {
	...
	const { data: colletData, loading: colletLoading } = useDashboardUserCollect(
		DashboardQueryType.claim,
	)
