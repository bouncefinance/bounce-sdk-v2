...
export function Activity({ poolInfo }: { poolInfo: FixedSwapNFTPoolProp }) {
	...
	const slicedBid1Amount = bidAmount
		? new BigNumber(bidAmount).times(poolInfo.ratio).toString()
		: ''
	const userToken1Balance = useCurrencyBalance(
		account || undefined,
			...
			<Box className="content">
				...
				<PoolCard>
					...
					<PoolInfoItem title="Unit Price" sx={{ marginTop: 12 }}>
						<Stack direction="row" spacing={8}>
							...
							<TokenImage
								...
								src={
									poolInfo.token0.largeUrl ||
									poolInfo.token0.smallUrl ||
									poolInfo.token0.thumbUrl ||
									DefaultNftIcon
								}
								size={20}
							/>
					...
					<PoolInfoItem sx={{ marginTop: 12 }} title="Bid amount limit">
						<Stack direction="row" spacing={6}>
							<Typography color={'#959595'}>
								{poolInfo.participant.swappedAmount0 || 0} NFT /{' '}
					...
					<PoolInfoItem sx={{ marginTop: 12 }} title="Claim At">
						<Stack direction="row" spacing={6}>
							<Typography color={'#959595'}>
								{new Date(poolInfo.claimAt * 1000).toLocaleString()}
							</Typography>
						</Stack>
					</PoolInfoItem>
				</PoolCard>
