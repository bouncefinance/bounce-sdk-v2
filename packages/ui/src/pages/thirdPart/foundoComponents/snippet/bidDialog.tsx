...
const BidDialog = ({ handleClose }: { handleClose: () => void }) => {
	...
	const token1Balance = useCurrencyBalance(
		account || undefined,
		poolInfo?.currentBidderMinAmount?.currency,
	)
	const [curSliderPer, setCurSliderPer] = useState(0)
	const minBidVal = useMemo(
		() => poolInfo?.currentBidderMinAmount,
		[poolInfo?.currentBidderMinAmount],
	)
				...
				>
					...
					>
						...
						>
							...
							{poolInfo?.currentBidderMinAmount?.toSignificant()}{' '}
							{poolInfo?.currentBidderMinAmount?.currency.symbol})
						</Typography>
						<InputBox>
							<input
								className={'inputEl'}
							...
							>
								<Button
									onClick={() =>
										bidHandler(
											poolInfo?.currentBidderMinAmount?.toSignificant(64, { groupSeparator: '' }) ||
												'',
										)
									}
									variant="outlined"
						...
						<Box mt={40}>
							<ProgressSlider
								isDark
								curSliderPer={curSliderPer}
								curSliderHandler={curSliderHandler}
							/>
						</Box>
						{poolInfo && (
							<BidButton bidVal={bidNum} handleClose={handleClose} poolInfo={poolInfo} />
						)}
					</Box>
				</Box>
			</Box>
...
function BidButton({
	...
	const { data: isUserInWhitelist, loading: whitelistLoading } = useIsUserInWhitelist(
		poolInfo,
		poolInfo.category,
	)
	const minBidVal = useMemo(
	...
	const bidAmount = useMemo(
		() =>
			poolInfo.currencyAmountMin1
				? CurrencyAmount.fromAmount(poolInfo.currencyAmountMin1.currency, bidVal)
				: undefined,
		[bidVal, poolInfo.currencyAmountMin1],
	)
	const {
		bidCallback,
		submitted: placeBidSubmitted,
		bidPrevGasFee,
	} = useEnglishAuctionPlaceBidCallback(poolInfo)
			...
			ret
				.then(() => {
					...
					show(DialogDarkTips, {
						...
						content: `You have successfully bid amount ${bidAmount.toSignificant()} ${
							poolInfo.token1.symbol
						}`,
					})
				})
	...
	const isInsufficientBalance = useMemo(() => {
		...
		bidVal,
		ethBalance,
		poolInfo.currencyAmountMin1?.currency.isNative,
		token1Balance,
	])
	...
	const [approvalState, approveCallback] = useApproveCallback(
		bidAmount,
		chainId === poolInfo.ethChainId
			? ENGLISH_AUCTION_NFT_CONTRACT_ADDRESSES[poolInfo.ethChainId]
			: undefined,
		true,
	)
	...
	const bidButton = useMemo(() => {
		...
		if (!isCurrentChainEqualChainOfPool) {
			return (
				<PlaceBidBtn onClick={() => switchNetwork(poolInfo.ethChainId)}>Switch Network</PlaceBidBtn>
			)
		}
		return (
			<Box>
