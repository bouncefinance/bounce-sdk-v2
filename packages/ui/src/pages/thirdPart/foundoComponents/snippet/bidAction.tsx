		...
		>
			...
			>
				...
				{poolStatus === PoolStatus.Live && (
					<UpcomingStatus text="Live" OpenAt={poolInfo?.closeAt || 0} />
				)}
			...
			{viewType === BidType.chartView && poolInfo && (
				<PriceChartView isDark showText={false} poolInfo={poolInfo} />
			)}
			<RowLabel
				style={{
					padding: '24px 0 32px',
		...
		<>
			{isWinner ? (
				...
				>
					...
					>
						Someone made a higher offer and your money is returned to your wallet with gas
						compensation.(Your Bid Amount:{' '}
						{poolInfo.participant.accountBidAmount?.toSignificant() || '-'} {poolInfo.token1.symbol}
						)
					</Typography>
				</Box>
			) : null}
			...
			ret
				.then(() => {
					...
					show(DialogTips, {
						...
						content: `You have successfully claim ${poolInfo.token0.symbol || 'NFT'} for ${
							poolInfo.name
						}`,
					})
				})
