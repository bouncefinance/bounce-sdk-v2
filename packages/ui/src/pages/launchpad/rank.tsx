...
export function Rank() {
	...
	function calcuE18(list: any[]) {
		return list.map((item: any) => {
			return {
				...item,
				custom: CurrencyAmount.ether(new BigNumber(Number(item.custom)).toFixed(0)).toSignificant(),
			}
		})
	}
	...
	return (
		...
		>
			<GrayButton
				...
				sx={{ position: 'absolute', left: '50px', top: '10px' }}
			>
				<ArrowBackIcon />
			</GrayButton>
			<H1 />
			...
			<Row gap={15} mt={38}>
				<Btn
					className={currentTab === 'contribution' ? 'select' : ''}
					onClick={() => setCurrentTab('contribution')}
				>
					BY CONTRIBUTION
				</Btn>
				<Btn
					className={currentTab === 'referrals' ? 'select' : ''}
...
const Btn = styled(Box)`
	background: linear-gradient(
		194.68deg,
		rgba(255, 239, 95, 0.1) 11.27%,
		rgba(179, 139, 61, 0.1) 90.4%
	);
