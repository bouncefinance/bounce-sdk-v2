...
const TypeitInfo4: { [key: string]: any } = {
	Tokens: {
		$TCOIN:
			'This point-like system allows users to earn points, incentivizing their typing activity.',
		'$WORD (mining token)':
	...
	for (const key in obj) {
		...
		if (typeof value === 'object') {
			...
			elements.push(
				<Box key={key} mt={20}>
					<Typography
						sx={{ fontSize: 18, fontWeight: Object.keys(TypeitInfo3).includes(key) ? 700 : 400 }}
					>
						{key} :{' '}
					</Typography>{' '}
					{innerElement}
			...
			elements.push(
				<Stack key={key} flexDirection={'row'} mt={5}>
					<Typography sx={{ width: { xs: 140, sm: 220 }, fontSize: 16, flex: 'none' }}>
						{key}
					</Typography>
					:
		...
		projectInfo: [
			{
				...
				info: [
					<span key={0}>
						Omega is building a real-yield multi level system. With OMG and veOMG people can earn
						double reward USDT and ETH. Our nontradeable veOMG people can earn double reward and
						participate in our dao decision making. We inplemented NFT boosted staking into our
						ecosystem so people can boost their earnings. Know more in details here:{' '}
						<Link
							color={'inherit'}
							href="https://omegaprjc.gitbook.io/omega-prjct/"
							target="_blank"
						>
			...
			},
			{
				...
				info: [
					<Link
						key={0}
						color={'inherit'}
						href="https://omegaprjc.gitbook.io/omg/ecosystem/tokenomics"
						target="_blank"
					>
						<img
							style={{ maxWidth: '100%' }}
