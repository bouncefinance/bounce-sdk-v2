...
export function Equilibria() {
	...
	const isLive =
		new Date().valueOf() > gameTimeStamp.start && new Date().valueOf() < gameTimeStamp.end
			...
			{status === StatusType.Rules && (
				...
				>
					...
					>
						...
						Play the game and try to get higher scores. The top 50 Ranked Players will receive a
						Whitelist to join stage 2.
					</Typography>
				</Box>
			)}
			...
			{status === StatusType.Warning && (
				...
				>
					...
					>
						...
						Check the auction creator, token contract and price. Bounce auction is a decentralized
						tool where anyone can launch.
					</Typography>
				</Box>
			)}
	...
	return (
		...
		>
			...
			>
				...
				>
					Equilibria Finance is designed exclusively for $PENDLE holders and liquidity providers,
					offering an easy-to-use platform to maximize your profits.
				</Typography>
				<Box
					sx={{
						display: 'flex',
		...
		<UserMainBlock
			...
			extraAlert={
				<Alert severity="warning">
					The $mEQB in this auction is the mirror token of $EQB. After the auction is over, the
					unlocking rules will be the same as the official. The official will redeem $mEQB according
					to the rules.
				</Alert>
			}
			contentGap={50}
