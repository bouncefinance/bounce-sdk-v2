	...
	const defaultActivation = useCallback(
		async (connector: (() => Promise<AbstractConnector>) | AbstractConnector | undefined) => {
			...
			if (
				...
				conn.walletConnectProvider?.connector?.connected
			) {
			...
			if (!window.web3 && !window.ethereum && option.mobile) {
				return (
					<Option
						...
						link={option.href}
						header={option.name}
						icon={getStaticAssetsUrl('@/assets/walletIcon/' + option.iconName)}
					/>
				)
...
export default function WalletModal({
	...
	//             link={option.href}
	//             header={option.name}
	//             icon={getStaticAssetsUrl('@/assets/walletIcon/' + option.iconName)?.default}
	//           />
	//         )
	//       }
	//       return null
