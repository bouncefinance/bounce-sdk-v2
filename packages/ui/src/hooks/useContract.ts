...
export function useSpaceIdRegistrarContract(
	withSignerIfPossible?: boolean,
	queryChainId?: ChainId,
): Contract | null {
	const { chainId } = useActiveWeb3React()
...
export function useFixedSwapERC20Contract(address?: string, queryChainId?: ChainId) {
	...
	const curAddress =
		address === '' ? undefined : address || (cur ? FIXED_SWAP_ERC20_ADDRESSES[cur] : undefined)
...
export function useRandomSelectionERC20Contract(address?: string, queryChainId?: ChainId) {
	...
	const curAddress =
		address === ''
			? undefined
			: address || (cur ? RANDOM_SELECTION_CONTRACT_ADDRESSES[cur] : undefined)
...
export function useFixedSwapNftContract(address?: string, queryChainId?: ChainId) {
	...
	const curAddress =
		address === ''
			? undefined
			: address || (cur ? FIXED_SWAP_NFT_CONTRACT_ADDRESSES[cur] : undefined)
...
export function useEnglishAuctionNftContract(address?: string, queryChainId?: ChainId) {
	...
	const curAddress =
		address === ''
			? undefined
			: address || (cur ? ENGLISH_AUCTION_NFT_CONTRACT_ADDRESSES[cur] : undefined)
