...
export * from './isInjectedConnectedPrev'
export * from './isZero'
...
export * from './retry'
export * from './triggerSwitchChain'
export * from './trimNumberString'
export * from './uriToHttp'
export * from './useDebouncedChangeHandler'
export * from './useResizeView'
...
export function getEtherscanLink(
	...
	return builder(
		SUPPORTED_NETWORKS[chainId]?.blockExplorerUrls?.[0].replace(/\/$/, '') || '',
		data,
		type,
	)
}
...
export function isSocialUrl(
	name: 'discord' | 'twitter' | 'github' | 'opensea' | 'youtube' | 'url',
	url: string,
) {
	switch (name) {
		case 'discord':
