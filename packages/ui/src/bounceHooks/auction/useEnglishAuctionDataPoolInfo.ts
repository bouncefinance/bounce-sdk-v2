...
function useEnglishAuctionAccountBidAmount(backedChainId?: number, poolId?: string) {
	...
	const rawAmount = useMemo(
		() => (account ? data?.list?.[0]?.token1Amount : undefined),
		[account, data?.list],
	)
...
export function useEnglishAuctionDataPoolInfo(backedId?: number) {
	...
	const {
		...
		run: getPoolInfo,
		loading,
	} = useBackedPoolInfo({ category: PoolType.ENGLISH_AUCTION_NFT, ...query, backedId })
	...
	const currentBidderAmount1 = useMemo(
		() => currentBidderAmount1Res?.[0].toString(),
	...
	const currentBidderMinAmount = useMemo(
		() => currentBidderMinAmountRes?.[0].toString(),
	...
	const data = useMemo(() => {
		...
		const _pools = {
			...
			currencyAmountMin1: poolsData.amountMin1
				? CurrencyAmount.fromRawAmount(t1, poolsData.amountMin1)
				: undefined,
		...
		const result: EnglishAuctionNFTPoolProp = {
			...
			participant: {
				...
				claimed: myClaimed,
				accountBidAmount: accountBidRawAmount
					? CurrencyAmount.fromRawAmount(t1, accountBidRawAmount)
					: undefined,
			},
			creatorClaimed: creatorClaimed || poolInfo.creatorClaimed,
			currentBidder: currentBidder,
			currentBidderAmount1: currentBidderAmount1
				? CurrencyAmount.fromRawAmount(t1, currentBidderAmount1)
				: undefined,
