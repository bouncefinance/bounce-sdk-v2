...
export const useBackedPoolInfo = (options?: PoolInfoParams) => {
	const {
...
export const usePoolInfo = (options?: PoolInfoParams) => {
	const {
		data: poolInfo,
		run: getPoolInfo,
		loading,
	...
	const v2FixedSwapData = useV2FixedSwapData(
		poolInfo?.poolVersion === 2,
		poolInfo?.poolId,
		poolInfo,
	)
...
function useV2FixedSwapData(
	...
	const fixedSwapERC20Contract = useMemo(
		() => (isV2 ? _fixedSwapERC20Contract : null),
		[_fixedSwapERC20Contract, isV2],
	)
	const { account } = useActiveWeb3React()
	...
	const queryReleaseDataListParams = useMemo(() => {
		...
		return Array.from({ length: Number(getReleaseDataListLengthRes[0]) }, (_, i) => i).map(
			(item) => [poolId, item],
		)
