...
export function useETHBalance(
	...
	return useMemo(
		() => (uncheckedAddress ? ETHBalances[uncheckedAddress] : undefined),
		[ETHBalances, uncheckedAddress],
	)
}
...
function useTokenBalancesWithLoadingIndicator(
	...
	const validatedTokens: Currency[] = useMemo(
		() =>
			tokens?.filter(
				(t?: Currency): t is Currency => isAddress(t?.address) !== false && !t?.isNative,
			) ?? [],
		[tokens],
	)
...
export function useCurrencyBalance(
	...
	console.log('🚀 ~ currency:', account, currency, chainId)
